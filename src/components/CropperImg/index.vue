	...
	<div class="cropper-content">
		<div class="cropper-box">
			...
			<div class="footer-btn">
				<div class="scope-btn">
					<label class="btn" for="uploads">Upload picture</label>
					<input type="file" id="uploads" style="position:absolute; clip:rect(0 0 0 0);"
						accept="image/png, image/jpeg, image/gif, image/jpg" @change="selectImg($event)">
					<el-button size="mini" type="info" plain icon="el-icon-zoom-in" @click="changeScale(1)">Zoom in
					</el-button>
					<el-button size="mini" type="info" plain icon="el-icon-zoom-out" @click="changeScale(-1)">Zoom out
					</el-button>
					<el-button size="mini" type="info" plain @click="rotateLeft">↺ Rotate left</el-button>
					<el-button size="mini" type="info" plain @click="rotateRight">↻ Rotate right</el-button>
				</div>
				<div class="upload-btn">
					<el-button size="mini" type="success" style="color: #FFFFFF;" @click="uploadImg('blob')">Confirm <i class="el-icon-upload"></i>
					</el-button>
				</div>
			</div>
		</div>
...
<script>
	...
	import {imgUpload} from '@/api/center.js'
	export default {
		name: "CropperImage",
		components: {
			VueCropper
		...
		methods: {
			...
			selectImg(e) {
				...
				if (!/\.(jpg|jpeg|png|JPG|PNG)$/.test(e.target.value)) {
					this.$message({
						message: 'Image type：jpeg、jpg、png',//图片类型要求
						type: "error"
					});
					return false
				}
			...
			uploadImg(type) {
				...
				if (type === 'blob') {
					...
					this.$refs.cropper.getCropBlob(async (data) => {
						...
						formData.append('file', data, "DJ.jpg")
						//调用axios上传
						// let {
						// 	data: res
						...
						// let {
						// 	data: res
						// } = await _this.$http.post(HOST+'/img/upload', formData)
						imgUpload(formData).then(res => {
							if (res.code === 200) {
								...
								_this.$message({
									message: 'There is some abnormal, please contact admin!',//文件服务异常，请联系管理员！
									type: "error"
								});
							}
						})
